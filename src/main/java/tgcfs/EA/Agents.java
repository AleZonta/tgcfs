            ...
            private synchronized void runClassifier(Algorithm model, Individual agent, Individual classifier, TrainReal totalInput, boolean real) throws Exception {
                ...
                if( decision>0.5 ) {
                    ...
                    if(real) {
                        ...
                        if (this.score) {
                            Scores sc = new Scores(agent.getModel().getId(),totalInput.getId(), classifier.getModel().getId(), 1d);
                    ...
                    if(real) {
                        ...
                        if (this.score) {
                            Scores sc = new Scores(agent.getModel().getId(), totalInput.getId(), classifier.getModel().getId(), 0d);
                            this.scores.addScore(sc);
                        }
                    }
                }
