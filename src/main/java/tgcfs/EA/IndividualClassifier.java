...
public class IndividualClassifier {
     ...
     */
    public IndividualClassifier(INDArray objPar, IndividualStatus ind){
     ...
     */
    public IndividualClassifier(INDArray objPar, IndividualStatus ind, boolean isSon){
        this.objectiveParameters = objPar;
        this.fitness = new AtomicDouble(0);
        this.model = null;
        this.myInputandOutput = new ArrayList<>();
     ...
     */
    public IndividualClassifier(int size) throws Exception {
     ...
     * @exception Exception if there are problems with the reading of the seed information
     */
    public IndividualClassifier(int size, IndividualStatus ind) throws Exception {
        //this.objectiveParameters = ThreadLocalRandom.current().doubles(size, -4.0, 4.0).collect(ArrayList::new,ArrayList::add, ArrayList::addAll);
        this.objectiveParameters = Nd4j.rand(1, size);
        for(int j = 0; j< size; j++){
            this.objectiveParameters.putScalar(j, RandomGenerator.getNextDouble(-1,1));
     ...
     * @exception Exception if there are problems with the reading of the seed information
     */
    public IndividualClassifier(int size, ENNClassifier model, IndividualStatus ind) throws Exception {
        this.objectiveParameters = Nd4j.rand(1, size);
        for(int j = 0; j< size; j++){
            this.objectiveParameters.putScalar(j, RandomGenerator.getNextDouble(-1,1));
        }
     ...
     * @param isSon boolean variable if the individual is a son
     */
    public IndividualClassifier(INDArray objPar, AtomicDouble fitness, ENNClassifier model, List<TrainReal> myInputandOutput, IndividualStatus ind, boolean isSon){
        this.objectiveParameters = objPar;
     ...
     */
    public void setModel(ENNClassifier model) {
        this.model = model;
    }
     ...
     */
    public IndividualClassifier deepCopy(){
        return new IndividualClassifier(this.objectiveParameters, new AtomicDouble(this.getFitness()), this.getModel().deepCopy(), this.getMyInputandOutput(), this.ind, this.isSon());
    }
